...
if exist "%LOCAL_SCRIPT%" (
    powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "%LOCAL_SCRIPT%" %*
    exit /b %errorlevel%
)

for /f "usebackq delims=" %%T in (
    `powershell.exe -NoProfile -NonInteractive -Command "$p = [System.IO.Path]::Combine([System.IO.Path]::GetTempPath(), 'pichu-' + [System.IO.Path]::GetRandomFileName().Replace('.','') + '.ps1'); $null = New-Item -ItemType File -Path $p -Force; $p"`
) do set "REMOTE_SCRIPT=%%T"

if not defined REMOTE_SCRIPT (
